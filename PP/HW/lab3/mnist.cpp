...
void LinearLayer(float *A, float *B, float *C, float *D, int n, int k, int m) {
    // A: [n,k], B: [k,m], C: [m], D: [n,m]
    // Parallelize over (i,j); keep inner 'a' sequential (good locality, simple)
    #pragma acc parallel loop gang present(A,B,C,D)
    for (int i = 0; i < n; i++) {
...
void Argmax(float *A, int *D, int n, int m) {
    // A: [n,m], D: [n]
    #pragma acc parallel loop gang present(A,D)
    for (int i = 0; i < n; i++) {
        float mx = A[i * m + 0];
        int index = 0;
        #pragma acc loop seq
...
// ---- my_nn: put everything in one data region ----
void my_nn(float *training_images_flat, int num_images,
    ...
    // Move data once: copyin inputs & weights; create intermediates; copyout results
    #pragma acc data \
        copyin(training_images_flat[0:num_images*LAYER0], \
               layer1_matrix[0:LAYER0*LAYER1], layer1_bias[0:LAYER1], \
               layer2_matrix[0:LAYER1*LAYER2], layer2_bias[0:LAYER2]) \
        create(layer1_output[0:num_images*LAYER1], layer2_output[0:num_images*LAYER2]) \
        copyout(result[0:num_images])
    {
